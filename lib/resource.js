...
var Backbone
  ...
  , permissible = function(collOrModel, data){
    ...
    app.log.info('resource: permission:', this.req.method + ' permission denied for ' + this.req.url + (this.req.user ? ' for ' + this.req.user.get('email') : ''))
...
Resource.prototype = {
  init: function(collection, options, callback){
    ...
    app.router.attach(function(){
      ...
      if (!this.isPermissible) this.isPermissible = function isPermissible(collOrModel, data){
        ...
        if (_.isFunction(permissions)) permissions = permissions.call(this, collOrModel || collection, data)
  ...
  , sse: function(){
    this.collection.resource = this.collection.resource || {}
    this.collection.resource.sse = new SSE(app.server, {
      path: path.join(_.result(this.collection, 'url'), '/subscribe')
    })

    this.collection.resource.sse.on('connection', function(client){
      ...
      client.ears.listenTo(this.collection, 'add', function(model){
        client.send('add', JSON.stringify(model.toJSON()))
      })
      client.ears.listenTo(this.collection, 'change', function(model){
        client.send('change', JSON.stringify(model.toJSON()))
      })
      client.ears.listenTo(this.collection, 'remove', function(model){
        client.send('remove', JSON.stringify({id: model.id}))
      })
    ...
    this.collection.on('add', function(model){
      ...
      model.resource.sse = new SSE(app.server, {
        path: path.join(_.result(model, 'url'), '/subscribe')
      })

      model.resource.sse.on('connection', function(client){
        var timeStart = Date.now()

        app.log.debug('router: sse: client connect:', {resource: this.name, model: model.id})

        client.ears = {}
        _.extend(client.ears, Backbone.Events)

        client.ears.listenTo(model, 'change', function(model){
          client.send('change', JSON.stringify(model.toJSON()))
        })
        client.ears.listenTo(model, 'destroy', function(model){
          client.send('destroy', JSON.stringify({id: model.id}))
        })

        client.on('close', function(){
          client.ears.stopListening()
          app.log.debug('router: sse: client disconnect:', {resource: this.name, model: model.id, timeConnected: Date.now() - timeStart})
        })
      }.bind(this))
    }, this)
  }
  ...
  , create: function(){
    ...
    collection.create(data, {
      error: function(model, err){
        app.log.error('resource: create: ' + name + ':', err)
        this.res.writeHead(500)
        this.res.json({code: 500, message: err})
      }.bind(this)
