  ...
  , sse: function(){
    var self = this
      , responder = function(client, collectionOrModel, e){
        client.ears.listenTo(collectionOrModel, e, function(model){
          ...
          })) client.send(e, JSON.stringify(['destroy', 'remove'].indexOf(e) > -1 ? {id: model.id} : model.toJSON()))
        })
      }
    ...
    this.collection.resource.sse = new SSE(app.server, {
      ...
      , verifyRequest: function(req){
        return self.getPermissions.call({req: req}, self.collection)
      }
    })
    ...
    this.collection.on('add', function(model){
      ...
      model.resource.sse = new SSE(app.server, {
        ...
        , verifyRequest: function(req){
          return self.getPermissions.call({req: req}, model)
        }
      })
