...
Resource.prototype.create = function(){
  ...
  if (createdModel.validationError) {
    app.log.warn('resource: create: ' + name + ': validation error:', {model: createdModel, err: createdModel.validationError, stack: new Error(createdModel.validationError).stack})
...
Resource.prototype.update = function(rawId){
  ...
  // set first, then save. This is effectively what `wait: true` should do, but backbone-associations doesn't set sub-values until after the save event if we just do a `wait: true`. Doing a `set` and then a `save` manually fixes the problem.
  // https://github.com/dhruvaray/backbone-associations/issues/72
  model.set(data)

  if (model.validationError) {
    app.log.warn('resource: update: ' + name + ': validation error:', {model: model, err: model.validationError, stack: new Error(model.validationError).stack})
    this.res.writeHead(422)
