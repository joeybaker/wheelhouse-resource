...
Resource.prototype = {
  ...
  , sse: function(){
    ...
    this.collection.SSE.on('connection', function(client){
      ...
      client.listenTo(this.collection, 'add', function(model){
        client.send('add', JSON.stringify(model.toJSON()))
      })
      client.listenTo(this.collection, 'change', function(model){
        client.send('change', JSON.stringify(model.toJSON()))
      })
      client.listenTo(this.collection, 'remove', function(model){
    ...
    this.collection.on('add', function(model){
      ...
      model.SSE.on('connection', function(client){
        ...
        client.listenTo(model, 'change', function(model){
          client.send('change', JSON.stringify(model.toJSON()))
        })
        client.listenTo(model, 'remove', function(model){
          client.send('remove', JSON.stringify({id: model.id}))
        })
