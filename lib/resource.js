  ...
  , sse: function(){
    var self = this
      , responder = function(client, collectionOrModel, e){
        client.ears.listenTo(collectionOrModel, e, function(model){
          // ensure the user is allowed to see this model
          var permissions = self.getPermissions.call(client, self.collection)

          app.log.debug('router: sse: send ' + e + ':', {resource: self.name, user: client.req.user})
    ...
    this.collection.resource.sse = new SSE(app.server, {
      ...
      , verifyRequest: function(req){
        var permitted = self.getPermissions.call({req: req}, self.collection)
        if (!permitted) app.log.warn('router: sse: client not permitted', {resource: self.name, user: req.user})
        return permitted
      }
    })
    ...
    this.collection.resource.sse.on('connection', function(client){
      ...
      ;['add', 'change', 'remove'].forEach(function(e){
        responder(client, this.collection, e)
      }.bind(this))
    ...
    this.collection.on('add', function(model){
      ...
      model.resource.sse = new SSE(app.server, {
        ...
        // ensure the user has access to this model
        , verifyRequest: function(req){
          var permitted = self.getPermissions.call({req: req}, model)
      ...
      model.resource.sse.on('connection', function(client){
        ...
        ;['change', 'destroy'].forEach(function(e){
          responder(client, model, e)
        })
