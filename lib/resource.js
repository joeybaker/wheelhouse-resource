...
Resource.prototype = {
  ...
  , sse: function(){
    this.collection.SSE = new SSE(app.server, {
      path: _.result(this.collection, 'url')
    })

    this.collection.SSE.on('connection', function(client){
    ...
    this.collection.on('add', function(model){
      model.SSE = new SSE(app.server, {
        path: _.result(model, 'url')
      })

      model.SSE.on('connection', function(client){
        var timeStart = Date.now()
