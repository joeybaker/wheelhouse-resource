...
var Backbone
  ...
  , permissible = function(collOrModel){
    ...
    app.log.warn('resource: permission:', this.req.method + ' permission denied', {url:this.req.url, user: (this.req.user ? this.req.user.get('email') : '')})
...
Resource.prototype.init = function(collection, options, callback){
  ...
  app.router.get(path.join('/', this.collection.url, '/([._a-zA-Z0-9-%]+)'), this.read)
  app.router.post(path.join('/', this.collection.url), this.create)
  app.router.put(path.join('/', this.collection.url, '/([._a-zA-Z0-9-%]+)'), this.update)
  app.router['delete'](path.join('/', this.collection.url, '/([._a-zA-Z0-9-%]+)'), this.del)
  this.sse()
...
Resource.prototype.getPermissions = function(collOrModel, data){
  ...
  if (_.isUndefined(permissions)) parsedPermissions = ['create', 'read', 'update', 'del']

  // build permissions if need be, assign to parsedPermissions so that we run the function on each request and don't override the permissions function on the resource
  else if (_.isFunction(permissions)) parsedPermissions = permissions.call(this, collection, data)
  else parsedPermissions = permissions

  // determine if this request is permissible
  // simple permissions
  if (_.isArray(parsedPermissions)){
    if (parsedPermissions.indexOf(methodMap[this.req.method]) > -1) return true
    else return false
  }
  // we didn't get anything back, so deny all permissions
  else if (!_.isObject(parsedPermissions)) return false
  // complex parsedPermissions
  else return parsedPermissions[methodMap[this.req.method]].call(this, model ? model.toJSON() : collection.toJSON(), data)
}

Resource.prototype.sse = function(){
  var responder, onConnection, sseResource
  ...
  sseResource = function createSseResource (collOrModel, events){
    ...
    collOrModel.resource.sse = new SSE(app.server, {
      ...
      , verifyRequest: function(req){
        ...
        if (req.user) meta.user = req.user.id
        if (!permitted) app.log.warn('router: sse: client not permitted:', meta)
        return permitted
      }
    })
