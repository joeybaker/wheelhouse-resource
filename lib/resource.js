...
Resource.prototype = {
  init: function(collection, options, callback){
    ...
    app.router.get(path.join('/', _.result(this.collection, 'url')), this.read)
    app.router.post(path.join('/', _.result(this.collection, 'url')), this.create)

    // routes to be attached for single models
    attachModelRoutes = function attachModelRoutes(model){
      app.router.get(path.join('/', _.result(this.collection, 'url'), '/' + encodeURIComponent(model.id)), this.read)
      app.router.put(path.join('/', _.result(this.collection, 'url'), '/' + encodeURIComponent(model.id)), this.update)
      app.router['delete'](path.join('/', _.result(this.collection, 'url'), '/' + encodeURIComponent(model.id)), this.del)
    }.bind(this)

    // if we have models in the collection, add their routes
    if (this.collection.length) this.collection.each(attachModelRoutes, this)

    // when a model is added to the collection, wait unitl it has an id, then add its routes
    this.collection.on('add', function(model){
      if (model.id) return attachModelRoutes(model)

      model.listenToOnce(model, 'change:id', attachModelRoutes.bind(this))
    }, this)

    // when a model is removed, kill the routes associated
    this.collection.on('remove', function(model){
      // split the url into an array
      var collUrl = _.result(this.collection, 'url').split('/')
        , routes

      // if the url started with a '/', the first value in the array will be blank, remove it.
      if (collUrl[0] === '') collUrl.shift()

      // find the model's routes on the router
      routes = _.reduce(collUrl, function(routeObject, key){
        if (_.has(routeObject, key)) return routeObject[key]
      }, app.router.routes, this)

      delete routes[encodeURIComponent(model.id)]
    }, this)
