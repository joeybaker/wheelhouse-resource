...
var Backbone
  ...
  , permissible = function(collOrModel){
    ...
    app.log.warn('resource: permission:', this.req.method + ' permission denied'
      , {url: this.req.url, user: (this.req.user ? this.req.user.get('email') : '')}
    )
...
Resource.prototype.sse = function(){
  ...
  responder = function(client, collectionOrModel, ev) {
    client.ears.listenTo.call(this, collectionOrModel, ev, function(model){
      ...
      else if (['destroy', 'remove'].indexOf(ev) > -1)
        client.send(ev, JSON.stringify({id: model.id}))
      })
  }
  ...
  app.router.get(path.join('/', this.collection.url, '/subscribe'), function(id){
    ...
    if (!resource.verifyRequest(this.req, resource.collection)) {
      this.res.writeHead(403)
      this.res.json({code: 403, message: 'No permission for ' + id})
      return
    }
  ...
  app.router.get(path.join('/', this.collection.url, '/([._a-zA-Z0-9-%]+)/subscribe'), function(id){
    var model
      , client

    model = resource.collection.get(decodeURIComponent(id))
    if (!resource.verifyRequest(this.req, model)) {
      this.res.writeHead(403)
      this.res.json({code: 403, message: 'No permission for ' + id})
      return
    }
...
Resource.prototype.verifyRequest = function(req, collOrModel){
  var permitted = this.getPermissions.call({req: req}, collOrModel)
    , meta = { resource: this.name }
